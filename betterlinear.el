...
(defcustom betterlinear-capture-buffer-name "*Linear Issue Capture*"
  "Base name of the temporary Org buffer used by `betterlinear-capture-issue'."
  :type 'string
  :group 'betterlinear)
...
(defvar-local betterlinear--project-stories-buffer-project-id nil
  "Linear project id used to refresh the current project stories buffer.")

(defvar-local betterlinear--project-stories-buffer-title nil
  "Org title used to refresh the current project stories buffer.")
...
(defvar betterlinear--project-stories-query
  "query BetterLinearProjectStories($id: String!, $first: Int!, $after: String) {
     project(id: $id) {
       id
       name
       url
...
(defvar betterlinear-capture-mode-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map org-mode-map)
    (define-key map (kbd "C-c C-c") #'betterlinear-capture-finalize)
    (define-key map (kbd "C-c C-k") #'betterlinear-capture-abort)
    map)
  "Keymap for `betterlinear-capture-mode'.")

(define-derived-mode betterlinear-capture-mode org-mode "BetterLinear Capture"
  "Major mode for temporary BetterLinear issue capture buffers.

Use `betterlinear-capture-finalize' (\\[betterlinear-capture-finalize]) to create
an issue in Linear, or `betterlinear-capture-abort'
(\\[betterlinear-capture-abort]) to discard the temporary buffer."
...
(defun betterlinear--revert-project-stories-buffer (&rest _args)
  "Refresh the current BetterLinear project stories buffer."
  (unless betterlinear--project-stories-buffer-project-id
    (user-error "No Linear project id associated with this buffer"))
  (betterlinear--insert-my-issues-org
...
(defun betterlinear--project-stories-page (project-id &optional after)
  "Return one page of Linear stories in PROJECT-ID after cursor AFTER."
  (let* ((variables `((id . ,project-id)
                      (first . ,betterlinear-issues-page-size)
                      (after . ,after)))
         (data (betterlinear--graphql betterlinear--project-stories-query variables))
...
(defun betterlinear--sort-order (issue)
  ...
  (let ((sort-order (alist-get 'sortOrder issue)))
    (cond
     ((numberp sort-order) sort-order)
     ((stringp sort-order) (string-to-number sort-order))
     (t nil))))

(defun betterlinear--sort-project-stories (stories)
  "Return STORIES ordered by Linear's project sort order.

Linear exposes the manual project order on each issue as `sortOrder'. When all
...
PROJECT-ID is the Linear project id. This follows Linear pagination until all
stories in the project have been retrieved. Linear exposes stories as issues in
its GraphQL API, so each returned story has the same shape as
`betterlinear-my-issues' entries.

When called interactively, prompt for a project and report how many stories were
retrieved. Use `betterlinear-project-stories-org' to display the stories in an
Org buffer."
  (interactive
   (let ((projects (betterlinear-projects)))
     (unless projects
       (user-error "No Linear projects found"))
     (list (alist-get 'id (betterlinear--read-project projects)))))
  (let (after stories has-next)
    (setq has-next t)
    (while has-next
      (let* ((page (betterlinear--project-stories-page project-id after))
             (nodes (plist-get page :nodes))
             (page-info (plist-get page :page-info)))
        (setq stories (nconc stories nodes))
        (setq has-next (eq (alist-get 'hasNextPage page-info) t))
        (setq after (alist-get 'endCursor page-info))))
    (setq stories (betterlinear--sort-project-stories stories))
    (when (called-interactively-p 'interactive)
...
TEAM is the Linear team selected before starting capture."
  (let ((team-id (betterlinear--string (alist-get 'id team)))
        (team-key (betterlinear--string (alist-get 'key team)))
        (team-name (betterlinear--string (alist-get 'name team))))
    (concat "#+TITLE: Capture Linear issue\n"
            "#+TODO: TODO\n"
            "#+STARTUP: showeverything\n\n"
            (format "# Team: %s%s\n"
                    (or team-key team-id "")
                    (if team-name (format " — %s" team-name) ""))
            "# Edit the heading and body.\n"
            "# Finish with C-c C-c; abort with C-c C-k.\n\n"
            "* TODO \n"
            ":PROPERTIES:\n"
            (if team-id (format ":TEAM_ID: %s\n" team-id) "")
            (if team-key (format ":TEAM: %s\n" team-key) "")
            ":END:\n\n")))

(defun betterlinear--goto-capture-heading ()
  "Move point to the issue heading in the current capture buffer."
  (goto-char (point-min))
  (unless (re-search-forward "^\\*+[[:space:]]+" nil t)
    (user-error "Capture buffer has no Org heading"))
  (beginning-of-line))

(defun betterlinear--kill-capture-buffer (buffer)
  "Kill temporary capture BUFFER and close one of its windows if displayed."
...
(defun betterlinear-capture-abort ()
  "Abort the temporary BetterLinear issue capture without creating anything."
  (interactive)
  (unless (derived-mode-p 'betterlinear-capture-mode)
    (user-error "This command must be run from a BetterLinear capture buffer"))
  (betterlinear--kill-capture-buffer (current-buffer))
  (message "Aborted Linear issue capture"))

;;;###autoload
(defun betterlinear-capture-issue (team)
  "Capture a new Linear issue in a temporary Org buffer.

Interactively, prompt for the Linear TEAM before opening the capture buffer.
Then edit the heading for the Linear title and add an optional description in
the body.
...
(\\[betterlinear-capture-abort]).  No permanent Org entry is written."
  ...
  (let ((buffer (generate-new-buffer betterlinear-capture-buffer-name)))
    (with-current-buffer buffer
      (betterlinear-capture-mode)
      (setq-local betterlinear--capture-team-id (alist-get 'id team))
      (insert (betterlinear--capture-buffer-template team))
      (betterlinear--set-local-todo-keywords '("TODO"))
      (set-buffer-modified-p nil)
      (betterlinear--goto-capture-heading)
      (end-of-line))
    (pop-to-buffer buffer)
    (message "Edit the issue, then C-c C-c to create it or C-c C-k to abort")))
...
(defun betterlinear-project-stories-org (project)
  "Retrieve all Linear stories in PROJECT and show them in Org.

Interactively, prompt for a project visible to the authenticated Linear user.
PROJECT is a Linear project alist, as returned by `betterlinear-projects'."
  (interactive
   (let ((projects (betterlinear-projects)))
     (unless projects
       (user-error "No Linear projects found"))
     (list (betterlinear--read-project projects))))
  (let* ((project-id (alist-get 'id project))
         (project-name (or (betterlinear--string (alist-get 'name project))
                           ...
                           "Project"))
         (title (format "Linear stories in %s" project-name))
         (buffer (get-buffer-create betterlinear-project-stories-buffer-name)))
    (with-current-buffer buffer
      (betterlinear-org-mode)
      (setq-local betterlinear--project-stories-buffer-project-id project-id)
      (setq-local betterlinear--project-stories-buffer-title title)
      (setq-local revert-buffer-function
                  #'betterlinear--revert-project-stories-buffer)
      (betterlinear--insert-my-issues-org
